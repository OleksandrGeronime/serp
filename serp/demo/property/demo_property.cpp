...
struct S1 : public serp::Service
{
    ...
    serp::Method::In<>::Bind<S1, &S1::foo> Foo{this, "Foo"};

    serp::Property<S1, int> Counter{0, "Counter"};
};
...
struct S2 : public serp::Service
{
    ...
    void init(const std::function<void(Service::Status)> reply) override
    {
        ...
        mS1->Counter.connect([](const int &value)
                             { serp::logMethod("S2::init::Counter.subscribe", value); });
